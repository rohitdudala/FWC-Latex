...
\item A manufacturer has three machine operators A, B and C. The first operator A produces 
...
\item A manufacturer has employed 5 skilled men and 10 semi-skilled men and makes two 
...
model $A$ is \rupee $15$ and on an item of model $B$ is \rupee $10$. How many of items of each model should be made per day in order to maximize daily profit ? Formulate the above LPP 
and solve it graphically and find the maximum profit.
\item Find the order and the degree of the differential equation $x^2 \frac{d^2y}{dx^2}=$
\[ 
\left\{
  \begin{array}{l}
    1+\frac{dy}{dx}^2 \\
...
\right\}
\]
\item If $f(x) = x + 7$ and $g(x) = x – 7$, $(x\in R)$ then find $\frac{d}{dx}(fog)(x)$.
...
$\begin{bmatrix}
    ...
    2 & 1 & 3    \\
    1 &–1 & 0   \\
\end{bmatrix},then find (A^2-5A).$
\item Find: $\int\frac{\tan^2x  \sec^2x}{1-\tan^6x}$ dx.
\item Solve ${\tan^{-1}2x}+{\tan^{-1}3x} = \frac{\pi}{4}$
\item If $log(x^2+y^2)=2\tan^{-1}(\frac{x}{y})$, show that $\frac{dy}{dx}=\frac{x+y}{x-y}$.
\item Using properties of determinants, prove the following 
$\begin{vmatrix}
    a + b + c &– c & – b    \\
    – c  & a + b + c  & – a \\
    – b  & – a  & a + b + c \\
\end{vmatrix} = 2(a + b) (b + c) (c + a)$.
\item $If x = \cos t + \log \tan(\frac{t}{2}),y=\sin t,$ then find the vales of $\frac{d^2y}{dt^2}$ and $\frac{d^2y}{dx^2}$ at $t= \frac{\pi}{4}$
\item If A=
$\begin{bmatrix}
    2 & –3 &  5 \\
    3 & 2  & – 4 \\
    1 & 1  & – 2 \\    
\end{bmatrix},then$ $find$ $A^{-1}$ .Hence solve the following system of equations :$2x – 3y + 5z = 11, 3x + 2y – 4z = –5, x + y – 2z = – 3.$
